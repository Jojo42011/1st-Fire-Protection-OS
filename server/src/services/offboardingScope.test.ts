...
test('offboarding includes the nine HR tasks and scopes items by department', () => {
  ...
  const hrCodes = new Set(['hr_notify_safety', 'hr_receive_devices', 'hr_icloud_logoff', 'hr_remove_pins', 'hr_vehicle_licensing', 'hr_sage_remove', 'hr_servicetrade_remove', 'hr_bamboo_inactivate', 'hr_empnav_terminate']);
  const hrItems = out.items.filter((i: any) => i.owner === 'hr');
  assert.equal(hrItems.length, 9);
  for (const c of hrCodes) assert.ok(out.items.some((i: any) => i.action_code === c), `${c} present`);

  // HR viewer sees only HR tasks; IT viewer sees only IT tasks; admin sees everything.
  const hrView = getOffboarding(id, ['hr'])!;
  assert.ok(hrView.items.length === 9 && hrView.items.every((i: any) => i.owner === 'hr'));
