...
const CATALOG: IntegrationDef[] = [
  ...
  },
  {
    ...
    why: 'Jobs, invoices & completions. Job completed + invoice created → arms the day-1/3/5/7 reminder sequence; also → review request.',
    baseline: 'planned',
  ...
  },
  {
    ...
    name: 'Resend (Email)',
    category: 'Messaging',
    why: 'Sends the automatic invoice reminders (day 1/3/5/7) and manual email nudges.',
    baseline: 'planned',
    isConnected: () => !!(process.env.RESEND_API_KEY && process.env.RESEND_FROM_EMAIL),
  },
  {
    id: 'telnyx',
    name: 'Telnyx (SMS)',
    category: 'Messaging',
    why: 'Texts the automatic invoice reminders (day 1/3/5/7) and manual SMS nudges.',
    baseline: 'planned',
    isConnected: () => !!(process.env.TELNYX_API_KEY && process.env.TELNYX_FROM_NUMBER),
  },
  {
    id: 'sms',
    name: 'SMS (Twilio)',
    category: 'Messaging',
    why: 'Alternate SMS transport for reminders + post-job review requests.',
    baseline: 'planned',
  ...
  },
  {
    ...
    category: 'Messaging',
    why: 'Alternate email transport for reminders + review-request emails.',
    baseline: 'planned',
