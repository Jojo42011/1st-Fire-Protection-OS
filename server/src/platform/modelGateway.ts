 ...
 * not OpenRouter itself, so a direct Anthropic/OpenAI/Google adapter can replace it later.
 */
export class OpenRouterGateway implements ModelGateway {
  constructor(
    private readonly apiKey = process.env.OPENROUTER_API_KEY || '',
    private readonly defaultModel = process.env.OPENROUTER_MODEL || 'anthropic/claude-sonnet-4.5',
  ) {}
